    ...
    export function Sidebar({ user, onLogout, onLinkClick }: SidebarProps) {
            ...
            <nav className="space-y-1 px-3">
              {userNavigation.map((item) => {
                ...
                return (
                  <button
                    key={item.name}
                    onClick={() => handleNavigation(item.href)} // ✅ use handler instead of <Link>
                    className={cn(
                      "flex items-center w-full rounded-lg px-3 py-2 text-sm font-medium transition-colors text-left",
                      isActive
                        ? "bg-blue-600 text-white"
                        : "text-gray-300 hover:bg-gray-800 hover:text-white"
                    )}
